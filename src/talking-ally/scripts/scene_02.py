...
class Interaction:
	...
	def findingLoop(self):
		...
		while True:
			key = self.okaomanager.getMaxAttentionKey("size")
			gazeLR = self.okaomanager.getAttension(key, "gazeLR")
			#print(gazeLR)
			x, y = self.okaomanager.getMaxAttensionFaceXY("size")
			if x == 0 and y == 0:
				pass
			else:
				self.address(x/2 + gazeLR*20, 800, 0.2)
			time.sleep(0.8)
	...
	def objectLoop(self):
		...
		while True:
			...
			if self.state == 0:
				# sitting...
				staytime += 1
				if staytime > staytime_max:
					staytime = staytime_max

				#print(staytime)
				if m - ave > 1300:
					self.state = 1
					if staytime == staytime_max:
						stateFunction[self.state](2.0)
						#stateFunction[self.state](2.0)
					else:
						#stateFunction[self.state](1.0)
						myabs = abs(1300-(m - ave))
						stateFunction[self.state](max(0.5, min(2.0, 0.1 + 1.5*myabs/(500))))
						#stateFunction[self.state](max(0.5, min(2.0, 0.1 + 1.5*myabs/(500))))
					time.sleep(1.1)
					staytime = 0
					staytime_max += 30000
	...
	def stateStanding(self, speed, update=True):
		...
		self.move(3, 0.0, speed, update)
		self.move(4, -0.4, speed, update)
		
		
		time.sleep(0.3)
		self.move(3, -0.2, 0.8, update)
		self.move(4, -0.6, 0.8, update)
		
		time.sleep(1.0)
		self.move(3, 0.0, 0.8, update)
		self.move(4, -0.1, 0.8, update)
